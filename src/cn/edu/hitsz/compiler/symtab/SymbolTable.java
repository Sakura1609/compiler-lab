...
public class SymbolTable {
    ...
    private final static Map<String, SymbolTableEntry> allowed = new HashMap<>();
    /**
     * 获取符号表中已有的条目
     *
     * @param text 符号的文本表示
    ...
    public SymbolTableEntry get(String text) {
        if (allowed == null || !allowed.containsKey(text)) {
    ...
    public SymbolTableEntry add(String text) {
        ...
        try {
            allowed.put(text, item);
        } catch (RuntimeException e) {
            throw new RuntimeException("符号已存在");
        }
        return item;
    }
    ...
    public boolean has(String text) {
        return allowed.containsKey(text);
    }
